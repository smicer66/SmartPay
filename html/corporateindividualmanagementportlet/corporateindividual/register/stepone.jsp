...
<portlet:actionURL var="proceedToStepTwo" name="processAction">
	<portlet:param name="action"
		value="<%=COMPANY_CREATE_INDIVIDUAL_ACTIONS.CREATE_AN_INDIVIDUAL_STEP_ONE.name()%>" />
</portlet:actionURL>

<div style="padding:10px; width:900px"> 	
    <div class="panel  panel-primary">
		<div class="panel-heading"><span style="color:white; font-weight: bold">Step 1 of 2: Create A Company Staff Profile</span></div>
		  ...
		  <select name="roletype" class="form-control">
			...
			<%if(userRoleTypeList!=null && userRoleTypeList.size()>0)
			{
				for(Iterator<RoleType> iter = userRoleTypeList.iterator(); iter.hasNext();)
				{
					...
					String selected = "";
					if(portletState.getSelectedCompany()!=null && portletState.getSelectedCompany().getCompanyType().equals(CompanyTypeConstants.COMPANY_TYPE_CORPORATE_COMPANY))
					{
						if((roleType.getRoleTypeName().equals(RoleTypeConstants.ROLE_TYPE_COMPANY_NON_ADMINISTRATOR_PERSONNEL)
				...
				<%
					...
					{
						if((roleType.getRoleTypeName().equals(RoleTypeConstants.ROLE_TYPE_RETAIL_STAFF)))
						{
							if(portletState.getSelectedUserRoleId()!=null && portletState.getSelectedUserRoleId().equals(Long.toString(roleType.getId())))
							{
								selected = "selected='selected'";
	 ...
	 %>
      ...
      <div style="padding-bottom:5px; clear:both"><div><strong>Primary Contact Mobile Number:<span style="color:red">*</span></strong></div>
	        <div>
	          ...
	          <input onkeypress="return onlyNumKey(event)" class="form-control" type="text" value="<%=portletState.getCorporateindividualfirstmobile()==null ? "" : (portletState.getCorporateindividualsecondmobile().length()>3 ? portletState.getCorporateindividualfirstmobile().substring(0, 3) : "") %>" name="contactMobileNumber" id="contactMobileNumber" placeholder="e.g. 9XXXXXXXXX" />
			  	</div>
	        </div>
	      </div>
	      <div style="padding-bottom:5px; clear:both"><div><strong>1st Alternative Contact Mobile Number:</strong></div>
	        <div>
				...
				<div style="float:left">
	          <input onkeypress="return onlyNumKey(event)" class="clear form-control" type="text" value="<%=portletState.getCorporateindividualsecondmobile()==null ? "" : (portletState.getCorporateindividualsecondmobile().length()>3 ? portletState.getCorporateindividualsecondmobile().substring(0, 3) : "") %>" name="contactMobileNumberFirstAlternative" id="contactMobileNumberFirstAlternative" placeholder="e.g. 9XXXXXXXXX" />
			  	</div>
	        </div>
	      </div>
	      <div style="padding-bottom:5px; clear:both"><div><strong>2nd Alternative Contact Mobile Number:</strong></div>
	        <div>
				...
				<div style="float:left">
	          <input onkeypress="return onlyNumKey(event)" class="clear form-control" type="text" value="<%=portletState.getCorporateindividualthirdmobile()==null ? "" : (portletState.getCorporateindividualthirdmobile().length()>3 ? portletState.getCorporateindividualthirdmobile().substring(0, 3) : "") %>" name="contactMobileNumberSecondAlternative" id="contactMobileNumberSecondAlternative" placeholder="e.g. 9XXXXXXXXX" />
			  	</div>
	        </div>
	      </div>
      
      <div style="clear:both"><strong>Primary Contact Email Address:<span style="color:red">*</span></strong>
        <div>
    ...
    All fields with red asterisk (*) imply they must be provided</div>
	  ...
	  {
	  %>
        <button name="createcorporateindividualStepOne" onclick="javascript:handleButton('back')" id="createcorporateindividualStepOne" class="btn btn-success" style="float:left">Go Back</button>
	  <%
	  }
	  %>
		...
		<input type="hidden" name="corporatestaffaction" id="corporatestaffaction" value="" />
		<input type="hidden" name="corporatestaffactionid" id="corporatestaffactionid" value="" />
...
{
	document.getElementById('corporatestaffaction').value=action;
