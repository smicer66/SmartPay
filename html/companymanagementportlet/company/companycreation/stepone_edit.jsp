<%@page import="com.google.zxing.common.Collections"%>
...
/*if(portletState.getPortalUser().getRoleType().getRoleTypeName().equals(RoleTypeConstants.ROLE_TYPE_BANK_ADMINISTRATOR))
{
	...
	if(portletState.getCompanyCRUDRights()!=null && portletState.getCompanyCRUDRights().equals(Boolean.TRUE))
		proceed = true;
	else
		proceed = false;
...
<portlet:actionURL var="proceedToStepTwo" name="processAction">
	<portlet:param name="action"
		value="<%=COMPANY_CREATION.EDIT_A_COMPANY_STEP_ONE.name()%>" />
</portlet:actionURL>


<div style="padding:10px; width:900px">
    <h2>
		Update Corporate Firm
	</h2>
    <form  id="startUpdateFormId" action="<%=proceedToStepTwo%>" method="post" enctype="application/x-www-form-urlencoded">
		<div style="padding:10px;">
			<div class="panel panel-info">
				<!-- Default panel contents -->
				<div class="panel-heading"><strong>Step 1 of 2: Provide New Company Details where necessary </strong></div>
					...
					<div style="padding:10px">
						<fieldset style="padding:10px">
						  ...
						  <div> <strong>Corporate Company Name:</strong>
							<div>
							  <input type="text" class="form-control" value="<%=portletState.getCompanyname()==null ? "" : portletState.getCompanyname() %>" name="companyName" id="companyName" placeholder="Provide The Company Name" />
							</div>
						  </div>
						  <div> <strong>Company Registration Number:</strong>
							<div>
							  <input onkeypress="return onlyNumKey(event)" type="text" class="form-control" value="<%=portletState.getCompanyrcnumber()==null ? "" : portletState.getCompanyrcnumber() %>" name="companyRCNumber" id="companyRCNumber" placeholder="Provide The Company Registration Number" />
							</div>
						  </div>
						  <div><strong>Contact Address Line 1:</strong>
							<div>
							  <input type="text" class="form-control" value="<%=portletState.getLine1addressofcompany()==null ? "" : portletState.getLine1addressofcompany() %>" name="contactAddressLine1" id="contactAddressLine1" placeholder="1st Line of Address" />
							</div>
						  </div>
						  <div><strong>Contact Address Line 2:</strong>
							<div>
							  <input type="text" class="form-control" value="<%=portletState.getLine2addressofcompany()==null ? "" : portletState.getLine2addressofcompany() %>" name="contactAddressLine2" id="contactAddressLine2" placeholder="2nd Line of Address" />
							</div>
						  </div>
						  <div><strong>Contact Mobile Number:</strong>
							<div>
							  <input onkeypress="return onlyNumKey(event)" type="text" class="form-control" value="<%=portletState.getCompanycontactphonenumber()==null ? "" : portletState.getCompanycontactphonenumber() %>" name="contactMobileNumber" id="contactMobileNumber" placeholder="Provide Contact Mobile Number" />
							</div>
						  </div>
						  <div><strong>Contact Email Address:</strong>
							<div>
							  <input type="text" class="form-control" value="<%=portletState.getCompanyemailaddress()==null ? "" : portletState.getCompanyemailaddress() %>" name="contactEmailAddress" id="contactEmailAddress" placeholder="Provide Contact Mobile Number" />
							</div>
						  </div>
						  <div>
							<div style="padding-bottom:5px; width:30%; float:left">
							  ...
							  <select name="bankBranch" id="bankBranch" class="form-control">
								...
								<%if(bankbranches!=null)
								{
									...
									%>
								<option <%=selected %> value="<%=bb.getId()%>"><%=bb.getName() + " - (" + bb.getBankCode() + ")"%></option>
								<%
									}
								} %>
							  </select>
							...
							<div style="padding-bottom:5px; width:50%; float:left">
							  ...
							  <input onkeypress="return onlyNumKey(event)" type="text" class="form-control" value="<%=portletState.getBankNumber()==null ? "" : portletState.getBankNumber() %>" name="accountNumber" id="accountNumber" placeholder="Type Bank Account Number" />
							</div>
						  </div>
						...
						All fields with red asterisk (*) imply they must be provided</div>
						  <div style="padding-top:20px; clear:both">
								<button class="btn btn-danger" id="cancelcreatecompany" style="float:left" onclick="javascript:handleButtonAction('caneledit'); return false;">Cancel</button>
								<button class="btn btn-success" id="createcompany" style="float:right" onclick="javascript:handleButtonAction('continueedit'); return false;">Continue</button>
						  </div>
						</fieldset>
					</div>
			  </div>
...
function handleButtonAction(value)
{
	...
	document.getElementById('startUpdateFormId').submit();
}
...
%>
	<jsp:include page="/html/access/accessrestricted.jsp" flush="" />
<%
}
%>
