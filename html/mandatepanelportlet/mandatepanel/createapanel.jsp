...
<portlet:actionURL var="bankbranchcreator" name="processAction">
	<portlet:param name="action"
		value="<%=AUTHORISATION_PANEL.CREATE_AUTH_PANEL.name()%>" />
</portlet:actionURL>


<div style="padding:10px;"> 	
    <div class="panel  panel-primary">
		<div class="panel-heading"><span style="color:white; font-weight: bold">
      ...
      {
      	%>Step 2 of 2: Authorisation Panel Details
		<%
      }else
      {
		%>
		Create An Authorisation Panel
		<%
      }
		%></span></div>
		    ...
		    <fieldset>
		    	<div style="padding:10px;">
			        ...
			        <div>
			          <select class="form-control" name="financialAmountRestriction" id="financialAmountRestriction">
						...
						if(portletState.getAllFinancialRestrictionsListing()!=null && portletState.getAllFinancialRestrictionsListing().size()>0)
						{
							...
							%>
								<option value="<%=far.getId()%>"><%=far.getName()%></option>
							<%
							}
						}
						%>
			      ...
			      <div> <strong>Type of Panel:</strong><span class="taglib-icon-help"><img alt="" aria-labelledby="dplx" onblur="Liferay.Portal.ToolTip.hide();" onfocus="Liferay.Portal.ToolTip.show(this);" onmouseover="Liferay.Portal.ToolTip.show(this);" src="/html/themes/control_panel/images/portlet/help.png" tabindex="0" aria-controls="aui_3_4_0_1_18202" id="aui_3_4_0_1_18192"><span class="aui-helper-hidden-accessible tooltip-text" id="dplx">Transaction Initiators initiate a transaction. Transaction Authorisers authorise a transaction</span></span>
			        <div>
			          <select class="form-control" name="panelType" id="panelType">
					  	<option value="-1">-Select A Financial Amount Threshold</option>
						<%
						String select2 = "";
						String select1= "";
						if(portletState.getPanelType()!=null && portletState.getPanelType().equals(PanelTypeConstants.AUTHORIZE_PANEL_TYPE_INITIATOR.getValue()))
						{
						...
						<%
						if(portletState.getPanelType()!=null && portletState.getPanelType().equals(PanelTypeConstants.AUTHORIZE_PANEL_TYPE_AUTHORISER.getValue()))
						{
			    ...
			    All fields must be provided</div>
			      <div>
			        <button name="createPanelName" id="createPanelName" class="btn btn-success">Create An Authorisation Panel</button>
			      </div>
				</div>
		    </fieldset>
		    </form>
