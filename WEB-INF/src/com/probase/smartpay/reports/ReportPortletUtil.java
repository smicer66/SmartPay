...
public class ReportPortletUtil {
	...
	public Collection<ApprovalFlowTransit> getApprovalFlowTransitListingsCreatedByRoleType(String workItemType, 
		...
		try {
				...
				String hql = "select rt from ApprovalFlowTransit rt where " +
						"lower(rt.portalUser.roleType.roleTypeName) = lower('" + roleType.getValue() + "') " +
	...
	public CompanyCRUDRights getCompanyCRUDRightsByPortalUser(
		...
		try {
				...
				String hql = "select rt from CompanyCRUDRights rt where rt.portalUser.id = " + pu.getId();
				log.info("Get hql = " + hql);
				rt = (CompanyCRUDRights) swpService.getUniqueRecordByHQL(hql);
	...
	public Collection<PaymentHistory> runPaymentHistoryHQL(String hql)
	{
	...
	public Collection<TaxType> getAllTaxTypes() {
		// TODO Auto-generated method stub
		Collection<TaxType> rt = null;
		
		try {
			
				rt = (Collection<TaxType>) swpService.getAllRecordsByHQL("Select tt from TaxType tt");
